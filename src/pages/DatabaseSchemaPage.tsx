...
export const DatabaseSchemaPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-colors">
        <div>
      ...
      <div className="theme-hero-gradient rounded-3xl p-6 sm:p-8 text-white shadow-md border border-slate-700/50 relative overflow-hidden">
        ...
        <div className="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="max-w-2xl space-y-2">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-xl bg-white/15 border border-white/20 text-emerald-300 flex items-center justify-center">
            ...
            <p className="text-xs sm:text-sm text-slate-100 dark:text-slate-200 leading-relaxed">
          ...
          <div className="bg-white/10 dark:bg-black/30 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-white/20 w-full lg:w-96 shrink-0 space-y-3">
            ...
            <div className="text-2xs text-slate-200 space-y-1.5">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-cyan-200">{testLockTime}</span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2 bg-white hover:bg-slate-100 text-slate-900 text-xs font-bold rounded-xl shadow-xs transition-all cursor-pointer flex items-center justify-center gap-1.5"
            >
              {testSimulationRun ? (
                <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {/* Pillar 1 */}
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-2.5 transition-colors">
          <div className="w-10 h-10 rounded-xl theme-bg-light theme-text-primary flex items-center justify-center">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">5 Core Modules</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {operationalModules.map((mod) => {
            ...
            return (
              <div
                key={mod.title}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs p-5 space-y-3 transition-colors"
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="w-9 h-9 rounded-xl theme-bg-light theme-text-primary flex items-center justify-center">
                  ...
                  <span className="px-2 py-0.5 rounded text-[10px] font-mono font-semibold uppercase bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                    {mod.tag}
                  </span>
                </div>
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 pt-2 space-y-1.5 text-xs">
                  {mod.fields.map((f) => (
                    <div
                      key={f.name}
                      className="p-2 rounded-lg bg-slate-50/80 dark:bg-slate-800/40 flex flex-col sm:flex-row sm:items-center justify-between gap-1"
                    >
                      <span className="font-semibold text-slate-800 dark:text-slate-200 font-mono">{f.name}</span>
                      <span className="text-slate-500 dark:text-slate-400 text-2xs sm:text-right">{f.desc}</span>
                    </div>
                  ))}
                </div>
              </div>
