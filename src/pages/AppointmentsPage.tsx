      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-[#101F3D] p-5 rounded-2xl border border-slate-200 dark:border-[#1C2E4C] shadow-xs transition-colors">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={onRefresh}
            title="Refresh list"
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl border border-slate-200 dark:border-[#243B53] text-slate-700 dark:text-slate-300 hover:bg-[#FAF8F3] dark:hover:bg-[#172B52] hover:text-slate-900 dark:hover:text-white text-xs font-medium transition-colors cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-[#FAF8F3] dark:bg-[#172B52]/60 border border-slate-200 dark:border-[#243B53] rounded-xl text-xs sm:text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:bg-white dark:focus:bg-[#172B52] focus:outline-none focus:ring-2 focus:ring-[#C43D27]/30"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full px-3 py-2 bg-[#FAF8F3] dark:bg-[#172B52]/60 border border-slate-200 dark:border-[#243B53] rounded-xl text-xs sm:text-sm text-slate-900 dark:text-white focus:bg-white dark:focus:bg-[#172B52] focus:outline-none focus:ring-2 focus:ring-[#C43D27]/30"
            >
        ...
        {(selectedStatus !== 'all' || selectedDoctor !== 'all' || selectedDate || searchQuery) && (
          <div className="flex justify-between items-center pt-2 border-t border-slate-100 dark:border-[#1C2E4C] text-xs">
            ...
            <button
              ...
              className="text-[#C43D27] dark:text-[#E05A44] hover:underline font-bold cursor-pointer"
            >
              Reset Filters
            </button>
          </div>
      ...
      <div className="bg-white dark:bg-[#101F3D] rounded-2xl border border-slate-200 dark:border-[#1C2E4C] shadow-xs overflow-hidden transition-colors">
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-slate-500 dark:text-slate-400">
            ...
            <p className="text-sm font-semibold text-slate-900 dark:text-slate-200">No appointments found</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              <thead>
                <tr className="bg-[#FAF8F3] dark:bg-[#172B52]/60 border-b border-slate-200 dark:border-[#1C2E4C] text-slate-500 dark:text-slate-400 uppercase text-2xs tracking-wider">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-[#1C2E4C]">
                {filtered.map((apt) => (
                  <tr key={apt.id} id={`apt-row-${apt.id}`} className="hover:bg-[#FAF8F3]/70 dark:hover:bg-[#172B52]/40 transition-colors">
                    {/* Patient Column */}
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      <div className="text-xs font-mono font-bold text-[#C43D27] dark:text-[#E05A44] flex items-center gap-1 mt-0.5">
                        <Clock className="w-3 h-3 text-[#C43D27] dark:text-[#E05A44]" />
                        <span>
                      ...
                      <span
                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-2xs font-bold uppercase tracking-wider font-mono ${
                          apt.status === 'confirmed'
                            ...
                            : 'bg-[#FAF8F3] dark:bg-[#172B52] text-slate-700 dark:text-slate-300'
                        }`}
                      >
                        {apt.status}
                      </span>
