    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#101F3D] rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-[#1C2E4C] shadow-xs relative overflow-hidden">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div>
            <div className="flex flex-wrap items-center gap-2.5 mb-2.5">
              <span className="px-3 py-0.5 rounded-full text-2xs font-mono font-bold bg-[#FAF8F3] dark:bg-[#172B52] text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-[#243B53]">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
            {isSessionActive ? (
              ...
              <button
                ...
                className="px-6 py-3.5 bg-[#C43D27] hover:bg-[#B03420] text-white font-bold text-sm rounded-2xl shadow-md hover:shadow-lg transition-all cursor-pointer flex items-center justify-center gap-2.5"
              >
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 dark:border-[#1C2E4C] grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
          ...
          <div className="md:col-span-2 bg-[#FAF8F3] dark:bg-[#172B52]/70 rounded-2xl p-4 border border-slate-200 dark:border-[#243B53] flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${
                  sessionState === 'listening'
                    ? 'bg-[#C43D27] text-white shadow-lg scale-105'
                    : sessionState === 'speaking'
                    ? 'bg-[#B03420] text-white shadow-lg scale-105'
                    : sessionState === 'processing'
                    ? 'bg-amber-600 text-white animate-spin'
                    : 'bg-slate-200 dark:bg-[#243B53] text-slate-500 dark:text-slate-400'
                }`}
              >
                {sessionState === 'speaking' ? (
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-900 dark:text-white">Call Status:</span>
                  {getStatusBadge(sessionState)}
                </div>
                <div className="text-2xs text-slate-500 dark:text-slate-400 font-mono mt-1">
                  {sessionState === 'listening'
          ...
          <div className="bg-[#FAF8F3] dark:bg-[#172B52]/70 rounded-2xl p-4 border border-slate-200 dark:border-[#243B53] flex items-center justify-between">
            <div className="space-y-1 text-2xs">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 font-medium">
                <ShieldCheck className="w-3.5 h-3.5 text-slate-700 dark:text-slate-300" />
                <span>Source of Truth:</span>
                <span className="font-bold text-slate-900 dark:text-white">Authoritative DB</span>
              </div>
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 font-medium">
                <Clock className="w-3.5 h-3.5 text-slate-700 dark:text-slate-300" />
                <span>Timezone:</span>
                <span className="font-bold text-slate-900 dark:text-white font-mono">Asia/Kolkata (IST)</span>
              </div>
            </div>
            ...
            {onNavigateToTab && (
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-white dark:bg-[#101F3D] text-slate-800 dark:text-slate-200 text-2xs font-bold border border-slate-200 dark:border-[#243B53] shadow-2xs hover:bg-[#FAF8F3] transition-colors cursor-pointer"
              >
                View Bookings
              </button>
            )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#101F3D] rounded-3xl border border-slate-200 dark:border-[#1C2E4C] shadow-xs flex flex-col h-[520px]">
          <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-[#1C2E4C] flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-[#C43D27] dark:text-[#E05A44]" />
            ...
            <div className="flex items-center gap-2">
              ...
              {transcripts.length > 0 && (
                <button
                  ...
                  className="p-1.5 rounded-lg text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:bg-[#FAF8F3] dark:hover:bg-[#172B52] transition-colors"
                >
          ...
          <div className="flex-1 p-4 sm:p-5 overflow-y-auto space-y-3.5 text-xs">
            {transcripts.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 dark:text-slate-500">
                <Bot className="w-10 h-10 mb-2 opacity-40 text-slate-400 dark:text-slate-400" />
                <p className="font-semibold text-slate-900 dark:text-slate-300">Voice Session Not Started</p>
                <p className="text-2xs max-w-sm mt-1 text-slate-500 dark:text-slate-400">
                  Click <strong className="text-slate-900 dark:text-white">"Start Voice Test"</strong> above and speak naturally through your microphone.
                </p>
              </div>
            ) : (
              transcripts.map((entry) => {
                if (entry.sender === 'user') {
                  return (
                    <div key={entry.id} className="flex justify-end">
                      <div className="max-w-[85%] bg-[#C43D27] text-white p-3 rounded-2xl rounded-tr-xs shadow-2xs space-y-1">
                        <div className="flex items-center justify-between gap-4 text-[10px] text-white/80">
                ...
                if (entry.sender === 'ai') {
                  return (
                    <div key={entry.id} className="flex justify-start">
                      <div className="max-w-[85%] bg-[#FAF8F3] dark:bg-[#172B52] text-slate-900 dark:text-slate-100 p-3 rounded-2xl rounded-tl-xs border border-slate-200 dark:border-[#243B53] shadow-2xs space-y-1">
                        <div className="flex items-center justify-between gap-4 text-[10px] text-slate-500 dark:text-slate-400">
                          <span className="font-bold flex items-center gap-1 text-slate-900 dark:text-slate-200">
                            <Bot className="w-3 h-3 text-[#C43D27] dark:text-[#E05A44]" /> CLINICFIRST AI Receptionist
                          </span>
                          <span className="font-mono">{entry.timestamp}</span>
                        </div>
                        <p className="text-xs leading-relaxed">{entry.text}</p>
                ...
                return (
                  <div key={entry.id} className="flex justify-center my-1">
                    <div className="px-3 py-1 rounded-full bg-[#FAF8F3] dark:bg-[#172B52] text-slate-600 dark:text-slate-400 text-2xs font-mono border border-slate-200 dark:border-[#243B53]">
                      {entry.text}
                    </div>
                  </div>
                );
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-[#101F3D] rounded-3xl p-5 border border-slate-200 dark:border-[#1C2E4C] shadow-xs">
            <h3 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider mb-1 flex items-center gap-1.5">
              <Stethoscope className="w-3.5 h-3.5 text-[#C43D27] dark:text-[#E05A44]" />
            ...
            <div className="space-y-2">
              {testScenarios.map((sc, i) => (
                <div
                  key={i}
                  className="p-2.5 rounded-xl bg-[#FAF8F3] dark:bg-[#172B52]/60 border border-slate-200 dark:border-[#243B53] space-y-1"
                >
                  <div className="flex items-center justify-between text-2xs">
                    <span className="font-bold text-slate-900 dark:text-slate-200">{sc.label}</span>
                    <span className="text-[10px] text-slate-400 font-mono">Spoken Input</span>
                  </div>
                  <div className="text-xs font-semibold text-slate-900 dark:text-white font-mono bg-white dark:bg-[#101F3D] p-1.5 rounded-lg border border-slate-200 dark:border-[#243B53]">
                    "{sc.text}"
                  </div>
          ...
          <div className="bg-white dark:bg-[#101F3D] rounded-3xl p-5 border border-slate-200 dark:border-[#1C2E4C] shadow-xs">
            <h3 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <UserCheck className="w-3.5 h-3.5 text-[#C43D27] dark:text-[#E05A44]" />
            ...
            <div className="space-y-2">
              {doctors.map((doc) => (
                <div
                  key={doc.id}
                  className="p-2.5 rounded-xl bg-[#FAF8F3] dark:bg-[#172B52]/60 border border-slate-200 dark:border-[#243B53] flex items-center justify-between text-xs"
                >
                  <div>
                    <div className="font-bold text-slate-900 dark:text-white">{doc.name}</div>
                    <div className="text-2xs text-slate-500 dark:text-slate-400">{doc.specialty}</div>
                  </div>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white dark:bg-[#243B53] text-slate-800 dark:text-slate-300 border border-slate-200 dark:border-[#334E68] font-semibold">
                    Active
                  </span>
                </div>
              ))}
