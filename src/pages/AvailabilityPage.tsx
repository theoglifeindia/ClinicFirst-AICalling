        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-xs sm:text-sm text-slate-900 dark:text-white focus:bg-white dark:focus:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-[#008768] dark:focus:ring-emerald-500"
            >
              {doctors.map((d) => (
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border rounded-xl text-xs sm:text-sm text-slate-900 dark:text-white focus:bg-white dark:focus:bg-slate-800 focus:outline-none focus:ring-2 ${
                isPastDate
                  ? 'border-rose-400 focus:ring-rose-500 focus:border-rose-500 bg-rose-50/40 dark:bg-rose-950/30'
                  : 'border-slate-300 dark:border-slate-700 focus:ring-[#008768] dark:focus:ring-emerald-500'
              }`}
            />
          </div>
          ...
          <div>
            <button
              ...
              className="w-full py-2.5 px-4 theme-btn-primary text-xs sm:text-sm font-bold rounded-xl transition-all shadow-xs cursor-pointer"
            >
              Check Availability
            </button>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs p-6 transition-colors">
            <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-800">
              ...
              {isPastDate ? (
                ...
                <span className="px-3 py-1 rounded-full text-xs font-mono font-bold theme-badge">
            ...
            {loading ? (
              <div className="py-16 text-center text-slate-500 dark:text-slate-400">
                <div className="inline-block animate-spin rounded-full h-5 w-5 border-2 border-slate-900 dark:border-slate-100 border-t-transparent mb-2" />
              ...
              <div className="py-12 text-center text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/60 rounded-xl my-4">
                <Calendar className="w-8 h-8 text-slate-300 dark:text-slate-600 mx-auto mb-2" />
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Doctor not scheduled on this day.</p>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                      {morningSlots.map((slot) => (
                        <button
                          ...
                          className="p-3 theme-bg-light hover:opacity-90 border border-slate-200/80 dark:border-slate-700 rounded-xl text-center group transition-all shadow-2xs cursor-pointer"
                        >
                          <div className="text-sm font-bold text-slate-900 dark:text-white font-mono">
                            {formatTo12Hour(slot)}
                          </div>
                          <span className="text-2xs theme-text-primary font-bold">
                            Available • Book
                          </span>
                        </button>
                      ))}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                      {eveningSlots.map((slot) => (
                        <button
                          ...
                          id={`avail-slot-${slot.replace(':', '-')}`}
                          onClick={() => onBookSlot(selectedDoctorId, date, slot)}
                          className="p-3 bg-sky-50/70 dark:bg-sky-950/40 hover:bg-sky-100 dark:hover:bg-sky-900/60 border border-sky-200 dark:border-sky-800/80 rounded-xl text-center group transition-all shadow-2xs cursor-pointer"
                        >
                          <div className="text-sm font-bold text-slate-900 dark:text-sky-100 font-mono">
                            {formatTo12Hour(slot)}
                          </div>
                          <span className="text-2xs text-sky-600 dark:text-sky-400 font-bold">
                            Available • Book
                          </span>
                        </button>
                      ))}
                  ...
                  <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {bookedSlots.map((slot) => (
                        <span
                          key={slot}
                          className="px-3 py-1 rounded-lg text-xs font-mono bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 line-through border border-slate-200 dark:border-slate-700"
                        >
                          {formatTo12Hour(slot)}
                        </span>
                      ))}
        ...
        <div className="space-y-4">
          <div className="bg-slate-950 text-slate-200 rounded-2xl p-5 border border-slate-800 shadow-md">
            ...
            <p className="text-2xs text-slate-400 mt-2">
              Exact JSON payload structure returned by <code className="text-emerald-300">getAvailableSlots(doctor_id, date)</code> for conversational LLMs & AI voice receptionists.
            </p>

            <pre className="mt-3 p-3 bg-slate-900 rounded-xl text-2xs font-mono text-emerald-300 overflow-x-auto border border-slate-800/80">
